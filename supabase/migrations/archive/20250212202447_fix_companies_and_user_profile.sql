-- Companies tablosunu düzelt
ALTER TABLE public.companies
ADD COLUMN IF NOT EXISTS user_id uuid REFERENCES auth.users(id);

-- Mevcut durumu kontrol et ve düzelt
DO $$
DECLARE
    user_id uuid;
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM public.companies WHERE user_id = user_id
    ) INTO company_exists;
    ...
    IF NOT user_exists THEN
        INSERT INTO public.users (
            ...
            now(),
            now(),
            (SELECT id FROM public.companies WHERE user_id = user_id)
        );
    END IF;
